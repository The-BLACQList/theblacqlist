...
Results (founder ran 2026-06-28; **Slow-4G + emulated Moto G Power** — worst-case lab; INP shown as Lighthouse TBT proxy):

| Page | Perf | LCP | CLS | TBT | Pass? |
|---|---|---|---|---|---|
| Home | 90 | 3.6s | 0 | 90ms | ✅ score (LCP = throttled lab) |
...
**LCP ~3.6s context (not a launch blocker):** measured under Slow-4G + low-end-phone emulation; Perf *scores* are 90–100. Cheap wins already in place (`.browserslistrc` modern, hero image `priority`). Remaining LCP is render-delay/CSS under throttling → post-MVP perf pass; validate real-user LCP via field data (CrUX/Sentry) after launch.
...
| 1 · Account deletion (live walk-through) | ✅ **PASS** | 2026-07-01 | Founder ran on prod. Verify SQL: (A) `auth.users` = 0 (account gone); (C) owned listing preserved with `owner_user_id`/`submitted_by`/`updated_by` = NULL (unclaimed). **Bug found + fixed along the way:** `auth.admin.deleteUser` failed ("Database error deleting user"). Real cause (Postgres log `42P01 relation "categories" does not exist`): trigger functions that fire during the delete cascade (`update_listing_save_count`, `update_listing_review_stats`, `update_listings_search_vector`, `refresh_listing_search_vector_from_details`) were `SECURITY INVOKER` — under `supabase_auth_admin`'s restricted search_path they couldn't resolve/write `public` objects. Fixed by making them `SECURITY DEFINER SET search_path = public` (migrations `20260630000000` + `20260701000000`) + a defensive service-role `saves` pre-delete in `deleteAccount.ts`. |
