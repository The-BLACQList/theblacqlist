...
export default async function EditProductPage({ params }: Props) {
  ...
  const [{ data: listings }, { data: product }] = await Promise.all([
    ...
    supabase
      ...
      .select(
        'id, name, description, price_cents, compare_at_price_cents, price_display_text, cover_image_url, tags, shipping_options, return_policy_note, external_purchase_url, status, listing_id'
      )
      .eq('id', productId)
      .maybeSingle(),
  ])
  ...
  const tagsValue = Array.isArray(product.tags) ? (product.tags as string[]).join(', ') : ''
      ...
      <div className="rounded-xl border border-charcoal/10 bg-white p-6">
        <ProductForm
          ...
          defaultValues={{
            ...
            compare_price_dollars: product.compare_at_price_cents
