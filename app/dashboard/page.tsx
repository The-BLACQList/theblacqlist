...
function CompletenessItem({ done, label }: { done: boolean; label: string }) {
  return (
    <li className="flex items-center gap-2">
      ...
      <span className={`font-body text-sm ${done ? "text-charcoal/60 line-through" : "text-charcoal/80"}`}>
        {label}
      </span>
    </li>
  )
    ...
    <div className="max-w-3xl space-y-6">
      ...
      {!listings || listings.length === 0 ? (
        <div className="rounded-xl border border-charcoal/10 bg-white px-6 py-12 text-center">
          <p className="font-body text-charcoal/60">No pages found. Contact support if you believe this is an error.</p>
        </div>
      ) : (
                ...
                <div className="px-5 py-4 border-b border-charcoal/8 flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className={`inline-block px-2 py-0.5 rounded-full font-subhead text-xs font-semibold ${
                      ...
                      <span className={`inline-block px-2 py-0.5 rounded-full font-subhead text-xs ${
