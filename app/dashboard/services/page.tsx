...
export default async function DashboardServicesPage() {
  ...
  const { data: rows } = listingIds.length > 0
    ? await supabase
        ...
        .select("id, name, global_slug, status, starting_price_cents, price_display_text, delivery_mode, listing_id")
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-charcoal/10 bg-pale-lavender/40">
                ...
                <th className="px-5 py-3 text-left font-subhead text-xs font-semibold text-charcoal/50 uppercase tracking-wide hidden md:table-cell">Delivery</th>
                <th className="px-5 py-3 text-left font-subhead text-xs font-semibold text-charcoal/50 uppercase tracking-wide hidden lg:table-cell">Price</th>
              ...
              {services.map((s) => (
                <tr key={s.id} className="hover:bg-pale-lavender/20 transition-colors">
                  ...
                  <td className="px-5 py-4 hidden lg:table-cell">
                    <p className="font-body text-xs text-charcoal/60">
                      {s.price_display_text ?? (s.starting_price_cents ? `Starting at ${formatPrice(s.starting_price_cents)}` : "—")}
                    </p>
                  </td>
                  <td className="px-5 py-4">
                    <span className={`inline-flex px-2 py-0.5 rounded-full font-subhead text-xs font-semibold ${STATUS_STYLES[s.status] ?? "bg-charcoal/10 text-charcoal/60"}`}>
                      {s.status}
                    </span>
                  </td>
                  <td className="px-5 py-4">
