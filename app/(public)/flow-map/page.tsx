...
export default async function FlowMapPage() {
  ...
  let personalImpact: {
    ...
    pendingCount: number
    topBusinesses: { listing_id: string; name: string; slug: string; amount_cents: number; receipt_count: number }[]
  } | null = null
  ...
  if (user) {
    ...
    let piBusinesses: { listing_id: string; name: string; slug: string; amount_cents: number; receipt_count: number }[] = []
      ...
      <div className="max-w-[800px] mx-auto space-y-10">
        ...
        <div className="pt-6">
          ...
          <p className="font-subhead text-sm text-charcoal/60 mt-3 max-w-[520px] leading-relaxed">
            Every receipt submitted to The BLACQList becomes an anonymized data point in our community dollar-flow map.
            No buyer names. No private data. Just the collective movement of dollars.
          </p>
        ...
        <div className="rounded-xl border border-charcoal/10 overflow-hidden">
          ...
          {!user ? (
          ...
          ) : personalImpact && personalImpact.approvedCount === 0 && personalImpact.pendingCount === 0 ? (
            <div className="bg-white px-5 py-8 text-center">
              <p className="font-subhead text-sm font-semibold text-brand-black">
                No receipts yet
              </p>
            ...
            <div className="bg-white px-5 py-5 space-y-4">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div>
                  <p className="font-subhead text-xs text-charcoal/50 uppercase tracking-wide">Total spent</p>
                  <p className="font-headline text-2xl text-brand-black mt-0.5">
                    {formatDollars(personalImpact.totalAmountCents)}
                  </p>
                </div>
                <div>
                  <p className="font-subhead text-xs text-charcoal/50 uppercase tracking-wide">Businesses</p>
                  <p className="font-headline text-2xl text-brand-black mt-0.5">
                    {personalImpact.uniqueBusinesses}
                  </p>
                </div>
                <div>
                  <p className="font-subhead text-xs text-charcoal/50 uppercase tracking-wide">Approved</p>
                  <p className="font-headline text-2xl text-brand-black mt-0.5">
                ...
                {personalImpact.pendingCount > 0 && (
                  <div>
                    <p className="font-subhead text-xs text-charcoal/50 uppercase tracking-wide">Pending</p>
                ...
                <div className="pt-3 border-t border-charcoal/5 space-y-2">
                  ...
                  {personalImpact.topBusinesses.map((b) => (
                    <div key={b.listing_id} className="flex items-center justify-between gap-3">
                      <p className="font-subhead text-sm font-semibold text-brand-black truncate">{b.name}</p>
        ...
        <div className="rounded-xl bg-white border border-charcoal/10 px-5 py-4">
          <p className="font-subhead text-xs font-semibold text-brand-black mb-1">Privacy by design</p>
          <p className="font-body text-xs text-charcoal/50 leading-relaxed">
            All dollar-flow data is anonymized. Individual receipt details and buyer identities are never included in
            public views. Community totals only appear when contributed to by 5 or more distinct transactions.
            Users can opt out of community aggregates at any time.
          </p>
        </div>
