...
export default async function VendorStorefrontPage({ params }: Props) {
  ...
  const [{ data: productRows }, { data: serviceRows }] = await Promise.all([
    serviceClient
      ...
      .select("id, name, global_slug, description, price_cents, compare_at_price_cents, price_display_text, cover_image_url, shipping_options, external_purchase_url, listing_id")
    ...
    serviceClient
      ...
      .select("id, name, global_slug, description, starting_price_cents, price_display_text, duration_text, delivery_mode, booking_url, cover_image_url, listing_id")
      .eq("listing_id", listing.id)
      .eq("status", "active")
      .order("created_at", { ascending: false })
      .limit(24),
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8">
          <nav aria-label="Breadcrumb" className="mb-3">
            <ol className="flex items-center gap-1.5 font-body text-xs text-charcoal/40">
              ...
              <li className="text-charcoal truncate max-w-[200px]" aria-current="page">{listing.name}</li>
            </ol>
          </nav>
        ...
        {(hasProducts || hasServices) ? (
            ...
            {hasProducts && (
              <section aria-labelledby="vendor-products-heading">
                <div className="flex items-center gap-2 mb-5">
                  ...
                  <h2 id="vendor-products-heading" className="font-headline text-xl text-brand-black">
                    Products
                  </h2>
                  <span className="ml-1 font-body text-xs text-charcoal/40">({products.length})</span>
                </div>
            ...
            {hasServices && (
              <section aria-labelledby="vendor-services-heading">
                <div className="flex items-center gap-2 mb-5">
                  ...
                  <h2 id="vendor-services-heading" className="font-headline text-xl text-brand-black">
                    Services
                  </h2>
                  <span className="ml-1 font-body text-xs text-charcoal/40">({services.length})</span>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
                  {services.map((s) => (
          ...
          <div className="rounded-xl border border-charcoal/10 bg-white py-20 text-center">
            ...
            <p className="font-body text-sm text-charcoal/50 mt-2 max-w-xs mx-auto">
              {listing.name} hasn&apos;t added any products or services to the marketplace yet. Check back soon.
            </p>
          </div>
        )}
