...
export default function ResetPasswordPage() {
  ...
  const confirmError =
    state && 'error' in state && state.field === 'confirmPassword' ? state.error : null

  return (
    <>
      <h1 className="font-headline text-[26px] text-brand-black mb-1">Set a new password</h1>
      ...
      <form action={action} noValidate className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1">
          ...
          {confirmError && (
            <p id="confirm-error" role="alert" className="text-xs font-subhead text-red-600 mt-0.5">
              {confirmError}
            </p>
          )}
        </div>
