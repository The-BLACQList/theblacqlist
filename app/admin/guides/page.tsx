    ...
    <div className="space-y-6">
      ...
      {items.length === 0 ? (
        <div className="rounded-xl border border-charcoal/10 bg-white px-6 py-12 text-center">
          ...
          <p className="font-body text-sm text-charcoal-soft mb-4">Create your first city guide.</p>
          <Link
            href="/admin/guides/new"
              ...
              {items.map((g) => (
                <tr key={g.id} className="hover:bg-[#f9f9fb] transition-colors">
                  <td className="px-4 py-3">
                    ...
                    <p className="font-body text-xs text-charcoal-faint mt-0.5">{g.slug}</p>
                  </td>
                  <td className="px-4 py-3 hidden md:table-cell">
                    <span className="font-body text-xs text-charcoal-soft">{g.city ?? '—'}</span>
                  </td>
                  <td className="px-4 py-3">
                    <span
