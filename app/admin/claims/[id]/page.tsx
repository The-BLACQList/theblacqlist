...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const { data } = await serviceClient.from("claims").select("listings!claims_listing_id_fkey(name)").eq("id", id).maybeSingle()
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="rounded-xl border border-charcoal/10 bg-white p-5">
            ...
            <dl>
              ...
              <Row label="User ID" value={
                <span className="font-mono text-xs text-charcoal/60 break-all">{claim.claimant_user_id ?? "—"}</span>
              } />
              <Row label="Role claimed" value={
                claim.role_at_business
                  ? <span className="capitalize">{ROLE_LABELS[claim.role_at_business] ?? claim.role_at_business}</span>
                  : null
              } />
          ...
          {/* Listing info */}
          <div className="rounded-xl border border-charcoal/10 bg-white p-5">
            <h2 className="font-headline text-base text-brand-black mb-3">Business being claimed</h2>
            {listing ? (
              <dl>
                <Row label="Name" value={listing.name} />
                <Row label="Type" value={<span className="capitalize">{listing.entity_type.replace(/_/g, " ")}</span>} />
          ...
          {!isReviewable && (
            <div className="rounded-xl border border-charcoal/10 bg-white p-5">
              ...
              <dl>
                <Row label="Reviewed by" value={reviewerProfile?.display_name ?? claim.reviewed_by ?? "—"} />
                <Row label="Reviewed at" value={claim.reviewed_at ? new Date(claim.reviewed_at).toLocaleString() : null} />
                {claim.rejection_reason && (
                  <Row label="Rejection reason" value={
                    <p className="whitespace-pre-wrap leading-relaxed">{claim.rejection_reason}</p>
                  } />
                )}
              </dl>
            </div>
            ...
            {isReviewable ? (
              ...
              <div className="rounded-lg bg-[#f5f5f7] px-4 py-3">
                <p className="font-subhead text-sm text-charcoal/60">
                  This claim is <strong>{claim.status.replace(/_/g, " ")}</strong>. No further action needed.
                </p>
              </div>
            )}
          </div>
