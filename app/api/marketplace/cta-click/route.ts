...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const { entity_type, entity_id, cta_type, listing_id, destination_url } = body as Record<
    string,
    unknown
  >
  ...
  if (!entity_type || !['listing', 'product', 'service'].includes(entity_type as string)) {
    return NextResponse.json(
      { error: 'Invalid entity_type.', code: 'VALIDATION_ERROR' },
      { status: 400 }
    )
  }
  if (!entity_id || typeof entity_id !== 'string') {
    return NextResponse.json(
      { error: 'entity_id is required.', code: 'VALIDATION_ERROR' },
      { status: 400 }
    )
  }
  if (!listing_id || typeof listing_id !== 'string') {
    return NextResponse.json(
      { error: 'listing_id is required.', code: 'VALIDATION_ERROR' },
      { status: 400 }
    )
  }
  if (
    !destination_url ||
    typeof destination_url !== 'string' ||
    !destination_url.startsWith('https://')
  ) {
    return NextResponse.json(
      { error: 'destination_url must be a valid https URL.', code: 'VALIDATION_ERROR' },
      { status: 400 }
    )
  }
  if (cta_type && typeof cta_type !== 'string') {
    return NextResponse.json(
      { error: 'Invalid cta_type.', code: 'VALIDATION_ERROR' },
      { status: 400 }
    )
  }
