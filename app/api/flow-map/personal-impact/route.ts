...
export async function GET() {
  ...
  if (error) {
    return NextResponse.json({ error: "Failed to load personal impact.", code: "SERVER_ERROR" }, { status: 500 })
  }
  ...
  let topBusinesses: { listing_id: string; name: string; slug: string; amount_cents: number; receipt_count: number }[] = []
