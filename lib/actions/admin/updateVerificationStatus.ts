...
export async function updateVerificationStatusAction(
  ...
  const decision = formData.get("decision")?.toString().trim() as "verified" | "rejected" | undefined
  ...
  if (listing.trust_tier === "unclaimed") {
    return { error: "This listing is unclaimed. It must be claimed before verification can be granted." }
  }
  ...
  void writeAuditLog({
    ...
    targetId: listingId,
    beforeState: { trust_tier: listing.trust_tier, verification_status: listing.verification_status },
