...
export async function approveClaimAction(
  ...
  const { data: claim } = await serviceClient
    .from('claims')
    .select(
      'id, status, claimant_user_id, listing_id, listings!claims_listing_id_fkey(id, name, trust_tier, owner_user_id)'
    )
    .eq('id', claimId)
    .maybeSingle()
  ...
  void writeAuditLog({
    ...
    beforeState: {
      ...
      listing_trust_tier: listing.trust_tier,
      listing_owner_user_id: listing.owner_user_id,
    },
    afterState: {
      claim_status: 'approved',
      listing_trust_tier: 'claimed',
      listing_owner_user_id: claim.claimant_user_id,
      notes,
    },
  })
  ...
  void (async () => {
    const { data: userData } = await serviceClient.auth.admin.getUserById(claim.claimant_user_id!)
