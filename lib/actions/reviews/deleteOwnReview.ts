...
export type DeleteOwnReviewState =
  | { success: true }
  | { error: string }
  | null
...
export async function deleteOwnReviewAction(
  ...
  if (review.reviewer_user_id !== user.id) return { error: "You do not have permission to delete this review." }
  if (review.status !== "intake") {
    return { error: "This review can no longer be deleted. Published reviews require admin removal." }
  }
