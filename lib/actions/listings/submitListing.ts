...
export async function submitListingAction(
  ...
  const serviceAreaDescription =
    formData.get("service_area_description")?.toString().trim() || null
  ...
  const socialInstagram =
    formData.get("social_instagram")?.toString().trim() || null
  const socialFacebook =
    formData.get("social_facebook")?.toString().trim() || null
  const socialTwitter =
    formData.get("social_twitter")?.toString().trim() || null
  const socialTiktok =
    formData.get("social_tiktok")?.toString().trim() || null
  const socialLinkedin =
    formData.get("social_linkedin")?.toString().trim() || null
  const socialYoutube =
    formData.get("social_youtube")?.toString().trim() || null
  ...
  if (
    !VALID_LOCATION_TYPES.includes(
      locationType as (typeof VALID_LOCATION_TYPES)[number]
    )
  ) {
  ...
  if (
    !VALID_CTA_TYPES.includes(ctaType as (typeof VALID_CTA_TYPES)[number])
  ) {
  ...
  if (listingError || !listing) {
    return {
      error: "Something went wrong creating your listing. Please try again.",
    }
  }

  // ── Create listing_details_business ────────────────────────────────────────
  ...
  if (detailsError) {
    // Attempt to clean up the orphaned listing
  ...
  return { success: true, listingId: listing.id, listingName: name }
}
