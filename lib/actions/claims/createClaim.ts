...
export async function createClaimAction(
  ...
  const verificationEmail =
    formData.get("verification_email")?.toString().trim() ?? ""
  const verificationPhone =
    formData.get("verification_phone")?.toString().trim() || null
  const roleAtBusiness =
    formData.get("role_at_business")?.toString().trim() ?? ""
  ...
  if (existingClaim) {
    return {
      error:
        "You already have a pending claim for this listing. View the status in your account.",
    }
  }
