...
function Label({ htmlFor, children, hint }: { htmlFor: string; children: React.ReactNode; hint?: string }) {
  return (
    <div>
      <label htmlFor={htmlFor} className="block font-subhead text-sm font-semibold text-brand-black mb-1">
        {children}
      </label>
...
export function CollectionAdminForm({ action, defaultValues, redirectOnSuccess }: CollectionFormProps) {
  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      <Field id="slug" label="Slug" hint="Auto-generated from title if left blank. Use only lowercase letters, numbers, and hyphens.">
      ...
      <label className="flex items-center gap-3 cursor-pointer">
        ...
        <span className="font-body text-sm text-brand-black">Active (visible on the public collections page)</span>
      </label>
  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      <Field id="author_name" label="Author">
        <input id="author_name" name="author_name" type="text" maxLength={100}
          defaultValue={defaultValues?.author_name ?? "The BLACQList Team"}
          className={inputCls} />
      </Field>

      <Field id="body" label="Body" hint="Use ## for headings, ### for sub-headings, > for block quotes. Double line breaks create new paragraphs.">
      ...
      <Field id="meta_description" label="Meta description" hint="Used for SEO. Keep under 160 characters.">
      ...
      <div className="flex flex-wrap justify-end gap-3 pt-2">
        ...
        {isPublished ? (
          <button type="submit" name="action" value="unpublish" disabled={isPending}
