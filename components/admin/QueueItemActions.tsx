...
export function QueueItemActions({ queueId, revalidatePath = "/admin/reports" }: Props) {
  const [resolveState, resolveDispatch, resolvePending] = useActionState(resolveQueueItemAction, null)
  const [dismissState, dismissDispatch, dismissPending] = useActionState(resolveQueueItemAction, null)

  const isPending = resolvePending || dismissPending
  const error = (resolveState as { error?: string } | null)?.error
